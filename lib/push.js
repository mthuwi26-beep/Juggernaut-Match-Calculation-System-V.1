...
// Cada tipo de evento usa su propio canal de notificaciones en Android — así
// el sonido y la vibración pueden ser distintos según qué pasó, incluso con
// la app cerrada (Android arma la notificación solo, sin pasar por el código
// de la app, así que el canal tiene que venir marcado acá).
const CANALES_ANDROID = {
  gol: "jmcs_gol",
  tarjeta: "jmcs_tarjetas",
  empieza: "jmcs_empieza",
  termina: "jmcs_termina",
  semaforo: "jmcs_semaforo",
};

export async function enviarPushFcm(token, payload) {
  try {
    const tipo = payload.data?.tipo;
    const canalAndroid = CANALES_ANDROID[tipo] || "jmcs_partidos";
    await getMessaging(appFirebaseAdmin()).send({
      ...
      data: payload.data || {},
      android: {
        notification: { channelId: canalAndroid },
      },
    });
    return { ok: true };
  } catch (err) {
